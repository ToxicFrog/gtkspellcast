...
local function spell(name)
	return function(gestures)
		return function(description)
			...
			gestures = { gestures:lower():gsub('-', ' '):gsub('%((.)', '%12'):split() }
			local spell = { name=name, desc=description, gest=gestures }
			table.insert(spellbook[#spellbook], spell)
			table.insert(spellbook.spells, spell)
		end
	end
end
